...
const SignUp = () => {
	...
	const handleSocialLogin = async (providerName) => {
		try {
			toast.loading(`Wait until you SignUp with ${providerName}`);
			setLoad(`Loading ${providerName}...`);
			
			const { signInWithPopup } = await import("firebase/auth");
			const { auth, googleProvider, githubProvider } = await import("../firebase");
			...
			fetch(`${import.meta.env.VITE_BACKEND_URL}/api/auth/social`, {
				...
				.then((json) => {
					...
					if (json.token) {
						...
						toast.error(json?.message || `${providerName} SignUp Failed`);
					}
				})
				.catch((error) => {
